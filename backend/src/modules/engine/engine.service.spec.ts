...
describe('EngineService', () => {
  ...
  const montar = async (llmHabilitado = true) => {
    ...
    llm = {
      canonizar: jest.fn().mockResolvedValue(null),
      habilitado: llmHabilitado,
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: EanEnricherService,
          useValue: {
            consultar: jest.fn().mockResolvedValue(null),
            habilitado: false,
          },
        },
      ],
  ...
  it('resolução por EAN → confiança máxima (0.99)', async () => {
    ...
    const r = await engine.canonizar({
  ...
  it('dicionário → 0.95; fallback → 0.3 (honestidade do score)', async () => {
    ocrAlias.resolverComEstagio.mockResolvedValueOnce({
      canonical: 'arroz',
      estagio: 'abreviacao',
    });
  ...
  it('não-ingrediente (guard): eh_alimento=false SEMPRE, mesmo se a resolução divagar', async () => {
    ocrAlias.resolverComEstagio.mockResolvedValue({
      canonical: 'sabonete liquido',
      estagio: 'fallback',
    });

    const r = await engine.canonizar({
      descricao: 'SABONETE LIQUIDO PROTEX 250ML',
    });
  ...
  it('cauda longa: fallback + IA habilitada → tier IA resolve e vira estagio "ia" (0.7)', async () => {
    ...
    expect(llm.canonizar).toHaveBeenCalledWith(
      'POLPA CUPUAC INTG 500G',
      undefined,
    );
  ...
  it('A6: propaga nucleo/especificador quando resolvido pela Camada 1 (núcleo+especificador)', async () => {
    ocrAlias.resolverComEstagio.mockResolvedValue({
    ...
    const r = await engine.canonizar({
      descricao: 'QUEIJO PROVOLONE QUATA FAT',
    });
    expect(r.produto_canonico).toBe('queijo provolone');
    expect(r.nucleo).toBe('queijo');
    expect(r.especificador).toBe('provolone');
  });

  it('A6: nucleo/especificador são null quando o estágio não os fornece (ex: EAN)', async () => {
    ocrAlias.resolverComEstagio.mockResolvedValue({
      canonical: 'creme de leite',
      estagio: 'ean',
    });
    ...
    expect(r.nucleo).toBeNull();
    expect(r.especificador).toBeNull();
  });

  it('corrigir() delega pro ocrAlias (flywheel: a base aprende)', async () => {
    await engine.corrigir(
      'LEITE CONDENSADO MOCA',
      'leite condensado',
      '7891000000000',
    );
    expect(ocrAlias.registrarCorreção).toHaveBeenCalledWith(
      'LEITE CONDENSADO MOCA',
      'leite condensado',
      '7891000000000',
    );
  ...
  it('cauda longa: item com EAN + resolução fraca → enricher aprende (estágio ean, 0.99)', async () => {
    ...
    var enricher = {
      consultar: jest
        .fn()
        .mockResolvedValue({
          produto_canonico: 'salgadinho',
          nome_completo: 'Salgadinho Pepe Legal',
          marca: 'Pepe Legal',
          fonte: 'openfoodfacts',
        }),
      habilitado: true,
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        { provide: OcrAliasService, useValue: ocrAlias },
        {
          provide: LlmCanonizadorService,
          useValue: { canonizar: jest.fn(), habilitado: false },
        },
    ...
    const r = await eng.canonizar({
      descricao: 'SALG PEPE LEGAL COSTELINHA',
      ean: '7896950800110',
    });
  ...
  it('canonizarLote preserva ordem e devolve todos', async () => {
    ...
    const r = await engine.canonizarLote([
      { descricao: 'ARR 5KG' },
